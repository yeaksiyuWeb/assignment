...
@section('content')
  ...
  <div class="container mt-5 mb-5">
    ...
    <div class="row">
      ...
      <form action="{{ route('posts.student.display') }}" method="GET" id="filterForm">
          @csrf
      ...
      @foreach ($posts as $post)
        @if($postType == 'allPosts')
          @can('viewAny', $post)
            ...
            <div class="modal fade" id="editFeed{{ $post['id'] }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form action="{{ route('posts.student.update', ['id' => $post['id']]) }}" method="POST">
                      @csrf 
                      @method('PUT')
            ...
            <div class="modal fade" id="createFeed" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <form action="{{ route('posts.student.store') }}" method="POST" class="container mt-4">
                    @csrf
            ...
            <form id="deleteForm{{ $post['id'] }}" action="{{ route('posts.student.destroy', ['id' => $post['id']]) }}" method="POST" style="display: none;">
                @csrf
                @method('DELETE')
            </form>
          @endcan
